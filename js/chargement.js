...
function ajouttexte(nombre) //Rajouter une vérif si les id existent
{
    ...
    if(nombre === 5)    //Fixation de session
    {
        ...
        partie2_contenu.innerHTML = "Pour reproduite cette faille, il faut utiliser du PHP et la fonction <code>setcookie()</code> qui permet de créer et stocker un cookie dans le navigateur de l'utilisateur. Pour pouvoir consulter un cookie, il faut passer par l'outil de développement en appyusant sur F12 ou CTRL+Maj+I. Ensuite il faut se rendre dans la partie stockage. Dans la page de démonstration, l'on peut trouver 3 cookies: un pour le mail, un autre pour le mot de passe et le pseudo de l'adminstrateur du site. Une autre manière de procéder est d'utiliser des cookies de session. La différence par rapport à la méthode expliquée précédemment est que les cookies sont reliés à une variable <code>$_SESSION</code> en php qui possède un identifiant unique."
       
        partie3_contenu.innerHTML ="Pour résoudre cette faille, il est possible d'utiliser la fonction session_regenerate_id() qui permet de regenérer l'identifiant d'une session de l'utilisateur. Cela permet alors d'actualiser les cookies de session. Une autre méthode est de détruire la session si elle parait suspecte avec la fonction <code>session_destroy()</code> en php. De ce fait les cookies liés à la session seront également détruits. Vérifier les informations liées à une session sont cohérentes entre-elles lorsqu'il parcourt un site par exemple est une bonne solution qui permet de protéger un site de potentiels autres failles."
    ...
    else if(nombre === 4)    //CSRF
    {
        ...
        partie3_contenu.innerHTML ="Une première protection serait de demander lors d'une action importante de l'utilisateur, comme par exemple procéder à un paiement, serait de lui demande de se reconnecter via un formulaire ou de valider un captcha. Une autre alternative est d'utiliser une instruction SameSite. Cette dernière utilise les cookies de session d'un site. Ces cookies sont envoyés au serveur à condition que ces derniers soit originiaire d'un site. Si ces cookies proviennent d'une autre source comme l'ordinateur de l'utilisateur, ils ne seront pas envoyés au serveur. Actuellement, cette fonctionnalité est supporté par Edge,Firefox,Chrome et Safari à partir de Safari 15."
    ...
    else if(nombre === 3)//Accès indirecte non autorisé
    {
        ...
        partie2_contenu.innerHTML = "Pour avoir un aperçu de cette faille, cliquer sur l'onglet demo (ou sur ce <a href='pages/pageacces.html' target= '_blank'>lien</a>)et remplacer l'URL pages/pageacces.html par pages/admin/pageacces.html. Pour reproduire cette faille, il faut ne jamais vérifier les droits que possède l'utilisateur.";
